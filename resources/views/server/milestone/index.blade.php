      ...
      <div class="card w-100">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h4 class="card-title mb-2">{{ $title ?? 'Data Client Management' }}</h4>
              <p class="card-subtitle text-muted mb-0">
                {{ $descriptionTitle ?? 'Kelola data dengan mudah dan efisien' }}
              </p>
            </div>
            <div>
              <a href="{{ route('create.milestone') }}" class="btn btn-primary "> <i class="bi bi-plus-lg"></i>
                Tambah</a>
            </div>
          </div>
            ...
            <table id="dataTable" class="table table-hover table-striped">
              <thead class="table-dark">
                <tr>
                  ...
                  <th scope="col" class="text-center" style="width: 10%">Deskrispi</th>
              ...
              <tbody>
                @forelse ($data as $item)
                  <tr>
                    ...
                    <th class="text-center">{{ $item->deskripsi ?? '' }}</th>
                    <td class="text-center">
                      <div class="btn-group" role="group">
                        <!-- Edit Button -->
                        <a href="{{ route('edit.milestone', $item->id) }}" class="btn btn-sm btn-outline-warning"><i
                            class="bi bi-pencil-square" style="font-size: 14px"></i></a>

                        <!-- Delete Button -->
                        <a href="{{ route('delete.milestone', $item->id) }}" title="Hapus Data"
